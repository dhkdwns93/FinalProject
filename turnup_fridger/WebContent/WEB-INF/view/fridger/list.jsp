...
$(document).ready(function () {
	...
	$("#searchByIdBtn").on("click", function(){
		$.ajax({
			"url":"/turnup_fridger/fridger/show/byId.do",
			/* "type" : "post", */
	        ...
	        "success": function(fridger){
	        	$("tbody").empty();
	        	 $("tbody").append($("<tr>").append($("<td>").append(fridger.fridgerId))
						 				...
						 				.append($("<td>").append($("<button>").prop("type","button").prop("id","joinBtn").append("JOIN")))
												 .append($("<td>").append($("<a>").prop("href","/turnup_fridger/fridger/update.do").append($("<button>").prop("type","button").append("UPDATE")))))
	...
	$("#searchByNameBtn").on("click", function(){
		$.ajax({
			"url":"/turnup_fridger/fridger/show/byName.do",
			/* "type" : "post", */
	...
	$("#searchByOwnerBtn").on("click", function(){
		$.ajax({
			"url":"/turnup_fridger/fridger/show/byOwner.do",
			/* "type" : "post", */
	...
	$(document).on("click","#joinBtn", function(){
		alert($(this).val());
		$.ajax({
			"url":"/turnup_fridger/fridger/join.do",
			"data":{fridgerId : $(this).val(), reqMemberId : "id-4"},
			"dataType":"text",
			"beforeSend":function(){	
