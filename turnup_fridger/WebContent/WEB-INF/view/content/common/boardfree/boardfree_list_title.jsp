...
<table class="table table-hover table-condensed" style="width:100%; border:1; text-align:center;">
	...
	<br><a style="text-align:center"><h2>검색한 제목이 없습니다.</h2></a>
</c:if>
...
<p style="text-align:center">
	<%-- ######################################################
			...
			###################################################### --%>
	<!-- 첫페이지로 이동 -->
	<a href="${initParam.rootPath}/common/boardfree/boardFreeByTitle.do?page=1&boardFreeTitle=${requestScope.boardFreeTitle}">첫페이지</a>
	...
	<c:choose>
		<c:when test="${requestScope.pageBean.previousPageGroup}">
			...
			<a href="${initParam.rootPath }/common/boardfree/boardFreeByTitle.do?page=${requestScope.pageBean.beginPage - 1}&boardFreeTitle=${requestScope.boardFreeTitle}">☜</a>
		</c:when>
		<c:otherwise>
				☜	
		</c:otherwise>
		...
		<c:forEach begin="${requestScope.pageBean.beginPage}" end="${requestScope.pageBean.endPage}" var="page">
			<c:choose>
				<c:when test="${requestScope.pageBean.page != page}"> <%-- 현재패이지가 아니라면 --%>
					<a href="${initParam.rootPath}/common/boardfree/boardFreeByTitle.do?page=${page}&boardFreeTitle=${requestScope.boardFreeTitle}">&nbsp;${page}&nbsp;</a>
				</c:when>
				<c:otherwise>
					&nbsp;[${page}]&nbsp;
				</c:otherwise>
	...
	<c:choose>
		<c:when test="${requestScope.pageBean.nextPageGroup}">
			...
			<a href="${initParam.rootPath }/common/boardfree/boardFreeByTitle?page=${requestScope.pageBean.endPage + 1}&boardFreeTitle=${requestScope.boardFreeTitle}">☞</a>
		</c:when>
		<c:otherwise>
				☞		
		</c:otherwise>
